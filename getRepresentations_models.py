...
def getReps(model, modelName):
    
    #output for representations
    outputDir = os.path.join(workingDir, "fastABX_materials", modelName)
...
model.eval()
getReps(model, "stt_ca-es_conformer_transducer_large")
...
model = model.to(device)
model.eval()
getReps(model, "stt_es_conformer_ctc_large")
